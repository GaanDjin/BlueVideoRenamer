...
namespace Blue_Video_Renamer
{
    ...
    public static class ShellUtilities
    {
        ...
        public static Dictionary<string, List<string>> GetFileDetails(string sFile)
        {
            //For writing?
            //Interop.DSOFile.dll
            ...
            {
                string detail = dir.GetDetailsOf(null, j);
            ...
            for (int i = 0; i < headers.Count; i++)
            {
                ...
                if (!toReturn.ContainsKey(headers[i]))
                    toReturn.Add(headers[i], new List<string>());

                toReturn[headers[i]].Add( det);
                //Console.WriteLine($"{i}\t{headers[i]}: {det}");
            }
            return toReturn;
